...
def calculate_arb(cycle, g, verbose=True):
    ...
    for (p1, p2) in zip(cycle, cycle[1:]):
        # print(p1, p2, g[p1][p2]["weight"]," ---> ", np.exp(-g[p1][p2]["weight"]))
        total += g[p1][p2]["weight"]
        print(f'{counter}) Convert {pnl} {p1} to {p2} for {np.exp(-total)} with convertion rate {np.exp(-g[p1][p2]["weight"])}')
        counter += 1
        pnl *= np.exp(-g[p1][p2]["weight"])
    arb = np.exp(-total) - 1
    if verbose:
        print("PATH:", cycle)
        print(f"PROFIT: {arb*100:.2g}%\n")
    return arb


def find_arbitrage( g, find_all=True, sources=None):
    """
